/* <comments */
function press_the_btn(froze_uuid){
	var the_btn = $('#btn_'+froze_uuid);
	var com_count = the_btn.attr('com_count');
	var add_comment_block = $('#add_comment_'+froze_uuid);
	var comments_block = $('#comments_'+froze_uuid);
	if (the_btn.attr('showed') == '0') {
		the_btn.html('<span class="glyphicon glyphicon-comment" aria-hidden="true"></span> Спрятать комментарии ('+com_count+')');
		the_btn.attr('showed', '1');
		the_btn.addClass('btn-default');
		the_btn.removeClass('btn-primary');
		/* add_comment_block.css({'display': 'block'});
		comments_block.css({'display': 'block'}); */
		load_add_comment_form(froze_uuid);
		load_comments(froze_uuid);
	} else {
		the_btn.html('<span class="glyphicon glyphicon-comment" aria-hidden="true"></span> Открыть комментарии ('+com_count+')');
		the_btn.attr('showed', '0');
		the_btn.removeClass('btn-default');
		the_btn.addClass('btn-primary');
...
$(document).ready(function() {
	if ( $('.comment_btn').length ) {
		var comments_count = $('.comment_btn').attr('com_count');
		if ( comments_count > 0 ) {
			var froze_uuid = $('.comment_btn').attr('frozeuuid');
			press_the_btn(froze_uuid);
		}
	}
});
function load_add_comment_form(froze_uuid){
	var add_comment_block = $("#add_comment_"+froze_uuid);
	$.ajax({
		type: "GET",
		url: "/comments/"+froze_uuid+"/add",
		data: "",
...
function load_comments(froze_uuid){
	var comments_block = $("#comments_"+froze_uuid);
	var the_btn = $("#btn_"+froze_uuid);
	if(the_btn.attr("showed") == "1"){
		$.ajax({
			type: "GET",
			url: "/comments/"+froze_uuid+"/list",
			data: "",
			success: function(data) {
...
function add_new_comment(froze_uuid, csrfmiddlewaretoken){
	var comments_block = $("#comments_"+froze_uuid);
	var content = encodeURIComponent($("#id_content_"+froze_uuid).val());
	$.ajax({
		type: "POST",
		url: "/comments/"+froze_uuid+"/list",
		data: "csrfmiddlewaretoken="+csrfmiddlewaretoken+"&content="+content,
		success: function(data) {
			comments_block.html(data);
			load_add_comment_form(froze_uuid);
			update_btn_count(froze_uuid);
		}
	});
}
function update_btn_count(froze_uuid){
	var the_btn = $("#btn_"+froze_uuid);
	if(the_btn.attr("showed") == "1"){
		$.ajax({
			type: "GET",
			url: "/comments/"+froze_uuid+"/count",
			data: "",
			success: function(data) {
				the_btn.html('<span class="glyphicon glyphicon-comment" aria-hidden="true"></span> Спрятать комментарии ('+data+')');
				the_btn.attr('com_count', data);
			}
		});
	}
}
...
/* <chiefcomments */
//Функционал, при нажатий кнопки 'Комментарии руководителя'
function press_the_chief_btn(froze_uuid){
	var the_btn = $('#chiefbtn_'+froze_uuid);
	var com_count = the_btn.attr('com_count');
	var add_comment_block = $('#add_chief_comment_'+froze_uuid);
	var comments_block = $('#chief_comments_'+froze_uuid);
	if (the_btn.attr('showed') == '0') {
		the_btn.html('<span class="glyphicon glyphicon-comment" aria-hidden="true"></span> Спрятать комментарии ('+com_count+')');
		the_btn.attr('showed', '1');
		the_btn.addClass('btn-default');
		...
		load_chief_comments(froze_uuid);
	} else {
		the_btn.html('<span class="glyphicon glyphicon-comment" aria-hidden="true"></span> Комментарии руководителя ('+com_count+')');
		the_btn.attr('showed', '0');
		the_btn.removeClass('btn-default');
		the_btn.addClass('btn-primary');
		add_comment_block.html('');
...
$(document).ready(function() {
	if ( $('.comment_chief_btn').length ) {
		var comments_count = $('.comment_chief_btn').attr('com_count');
		if ( comments_count > 0 ) {
			var froze_uuid = $('.comment_chief_btn').attr('frozeuuid');
			press_the_chief_btn(froze_uuid);
		}
	}
});
//Загрузить форму для комментариев
function load_add_chief_comment_form(froze_uuid){
	var add_comment_block = $("#add_chief_comment_"+froze_uuid);
	$.ajax({
		type: "GET",
		url: "/comments/"+froze_uuid+"/add_chief_comment",
		data: "",
		success: function(data) {
			add_comment_block.html(data);
		}
...
//Выгрузить список комментариев
function load_chief_comments(froze_uuid){
	var comments_block = $("#chief_comments_"+froze_uuid);
	var the_btn = $("#chiefbtn_"+froze_uuid);
	if(the_btn.attr("showed") == "1"){
		$.ajax({
...
function add_new_chief_comment(froze_uuid, csrfmiddlewaretoken){
	var comments_block = $("#chief_comments_"+froze_uuid);
	var content = encodeURIComponent($("#id_content_"+froze_uuid).val());
	$.ajax({
		type: "POST",
...
function update_chiefbtn_count(froze_uuid){
	var the_btn = $("#btn_"+froze_uuid);
	if(the_btn.attr("showed") == "1"){
		$.ajax({
			type: "GET",
...
function ooo_reforma(){
	...
	if(the_value == 'ooo_reforma_plus_s_nds' || the_value == 'ooo_reforma_plus_bez_nds' || the_value == 'ooo_reforma_sever_s_nds' || the_value == 'ooo_reforma_sever_bez_nds' || the_value == 'ooo_reforma_sever' || the_value == 'ooo_reforma_plus' || the_value == 'iskusstvenny_kamen_ooo_re_forma_plus' || the_value == 'ooo_reforma_plus_tehnika' || the_value == 'montazh_demontazh_ooo_reforma_plus_yur' || the_value == 'podklyuchenie_2021_06_tehniki_ooo_reforma_plus_yur' || the_value == 'transportation_services_ooo_reforma_plus_yur' || the_value == 'entity_furniture_making_reforma_plus_ooo' || the_value == 'juridical_furniture_making_fabrika_mebeli_reforma' || the_value == 'entity_householdtec_reforma_plus_ooo' || the_value =='juridical_householdtec_fabrika_mebeli_reforma' || the_value == 'entity_connectiontec_reforma_plus_ooo' || the_value == 'juridical_connectiontec_fabrika_mebeli_reforma' || the_value == 'entity_mattresses_reforma_plus_ooo' || the_value == 'juridical_mattresses_fabrika_mebeli_reforma'  || the_value == 'entity_finishedfur_reforma_plus_ooo' || the_value == 'juridical_finishedfur_fabrika_mebeli_reforma' || the_value == 'entity_stone_reforma_plus_ooo' || the_value == 'juridical_stone_reforma_fabrika_mebeli_reforma' || the_value == 'entity_textile_reforma_plus_ooo' || the_value == 'juridical_textile_reforma_fabrika_mebeli_reforma' || the_value == 'entity_transport_reforma_plus_ooo' || the_value == 'juridical_transport_reforma_fabrika_mebeli_reforma' || the_value == 'entity_assembling_reforma_plus_ooo' || the_value == 'juridical_assembling_reforma_fabrika_mebeli_reforma' || the_value == 'entity_furnitureassemblywork_reforma_plus_ooo' || the_value == 'juridical_furnitureassemblywork_fabrika_mebeli_reforma' || the_value == 'entity_ufa_furniture_making_fabrika_mebeli_reforma' || the_value == 'entity_ufa_householdtec_fabrika_mebeli_reforma' || the_value == 'entity_ufa_connectiontec_fabrika_mebeli_reforma' || the_value == 'entity_ufa_mattresses_fabrika_mebeli_reforma' || the_value == 'entity_ufa_carpets_and_rugs_fabrika_mebeli_reforma' || the_value == 'entity_ufa_stone_fabrika_mebeli_reforma' || the_value == 'entity_ufa_finishedfur_fabrika_mebeli_reforma' || the_value == 'entity_ufa_textile_fabrika_mebeli_reforma' || the_value == 'entity_ufa_transport_fabrika_mebeli_reforma' || the_value == 'entity_ufa_assembling_fabrika_mebeli_reforma'){
